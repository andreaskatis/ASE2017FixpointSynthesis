...
The technique that we presented in this paper follows similar directions of previous work on abductive inference~\cite{dillig2013inductive, dillig2014optimal}. Dillig \textit{et al.} showed how abduction can be effectively used in invariant generation and guard synthesis. Their approach focuses on checking the validity of quantifier-free formulas of the form $\phi \implies \psi$, using a refinement process that generates candidate invariants in the form of maximum universal subsets (MUS). While the most general candidate may be sufficient to prove validity, it may also mislead the invariant search. For this reason, the authors use a backtracking procedure that effectively discovers new subsets while avoiding spurious results. Comparing this work to our case, the problem of synthesizing reactive systems involves the use of mixed-quantifier formulas, which are not supported, while the regions of validity that we generate are maximal and by construction cannot mislead the refinement process into unsound results, therefore making the use of any backtracking unnecessary. Most impontantly, in the context of reactive synthesis, a modification in the MUS algorithm would be necessary, since it is crucial that the generated subsets do not impose constraints over input variables. In our work, the ability to generate precise regions of validity lead to a very simple refinement process that is solely based on the input variables being existentially quantified.
