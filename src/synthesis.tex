...
safety-critical systems, and is the kernel language of SCADE, a popular 
model-based development tool. The contract is defined as a Lustre node, with a global
...
namely \texttt{e}, that is in fact used as the output of the system that we wish to
implement and represents Cinderella's choice at each of her turns.
...
viable states that can use the transition relation to reach new viable states that comply with the specification. Initially $F(s) = true$, and we ask \aeval for the validity of the formula $\phi \gets \forall s,i. \ (F(s) \land A(s,i) \Rightarrow \exists s'.G_{T}(s,i,s') \land F(s'))$. Unfortunately, not every state in $F(s)$
