...
viable states that can use the transition relation to new viable states that comply with the specification.

 Initially $F(s) = true$, and we ask \aeval for the validity of the formula $\phi \gets \forall s,i. \ (F(s) \land A(s,i) \Rightarrow \exists s'.G_{T}(s,i,s') \land F(s'))$. Unfortunately, not every state in $F(s)$
satisfies the formula. A representative counterexample returned by \aeval in this case is the set of assignments $\textit{cex} = \{\textit{pre}(b_{4}) = 3025, i_{4} = 0.2, b_{4} = 3025.2\}$, where the already overflown bucket $b_4$ receives additional water during the transition to the next state, violating the contract guarantees. Since $\phi$ is not valid, \aeval provides us with
a region of validity $Q(s,i)$, which is a subset of $F(s) \land A(s,i)$ for
which the formula $\phi$ is valid, blocking thus the inclusion of violating states such as the one described by $cex$. Since $Q(s,i)$ is defined over both state and
...
proof of the contract's realizability. As a counterexample \aeval returns the set of assignments $\textit{cex'} = \{b_1 = b_2 = b_3 = 16/11, b_4 = 17/11, b5 = 12/11\}$, which indicates that there are still non-violating states that may lead to a fixpoint. For the sake of completeness, if $\phi'$ was valid, then every state in
$F(s)$ is unsafe, under a specific input that satisfies the contract
assumptions (since $\lnot Q(s,i)$ holds in this case), and the specification is unrealizable. This is not the case,
however, and \aeval computes a region of validity $W(s)$ that describes states that are unsafe under certain valid inputs. As a consequence, a refinement process comes at the next step. in which we derive $F(s) = F(s) \land \lnot W(s)$, thus blocking the unsafe region $W(s)$.
