...
over the contract's inputs over and above what are required by $A$, ultimately leading to implementations that only work correctly for a small part of the input domain.
...
the existence of unsafe states in $F(s)$ for some input.  We remove this unsafe region from $F$ in line 16.  

The loop continues until we reach a (greatest) fixpoint for $F$.  If the system is unrealizable, this fixpoint (which may be the formula `false') does not intersect the set of initial states.  
