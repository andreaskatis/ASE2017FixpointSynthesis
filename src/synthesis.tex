...
\Procedure{\jsynvg}{A : assumptions, G : guarantees}
	...
	\While{true}
		...
		\If{$(\isValid(aevalResult))$}
			\State $\skolems \gets (aevalResult.\skolems)$;
			\If{$G_{I}(s) \land F(s) \neq false$}
				\Return \realizable, %\textcolor{red}{\init},
		 		\skolems;
			\Else{}
		 		\Return \unrealizable;\Comment{Empty set of initial states}
		 	\EndIf
		\Else
			\State $Q(s,i) \gets aevalResult.validSubset$;\Comment{Extract region of
			validity $Q(s,i)$}
			\If{$Q(s,i) = false$}
				\Return \unrealizable;
			\EndIf
			...
			Q(s,i)$;
			\State $aevalResult' \gets \aeval(\phi')$;
			\If{$(\isValid(aevalResult'))$}\Comment{Check if $Q(s,i)$ contains only
			unsafe states}
				\State $\skolems \gets (aevalResult'.\skolems)$;
				\Return \unrealizable, \skolems;\Comment{Extract witnessing cex}
			\Else{}
