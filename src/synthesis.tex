...
(unrealizability) of specification in Sections~\ref{sec:soundness} and~\ref{sec:termfinal}. The
example from Section~\ref{sec:example} is used in Section~\ref{sec:algexample} to illustrate the
...
the existence of unsafe states in $F(s)$ for some input.  We remove this unsafe region from $F$ in line 16.   The loop continues until we reach a greatest fixpoint for $F$.  %If the system is unrealizable, this fixpoint (which may be the formula `false') does not intersect the set of initial states.

%or until $W(s)$ becomes valid, that is, for any assignment of the state variables $s$, we have an input that satisfies the assumptions and violates $Q$.  In this case, every state in $F$ has some failing input and the system is unrealizable, which will lead to a failed check in Line 7 during the next iteration of the loop.
...
% $F(s) = \viable(s)$. Therefore the check on Line 7 is equivalent to
...
We make an explicit distinction on which node arguments are the system's inputs, using the \textit{REALIZABLE} statement towards the end of the contract. The contract's
...
undesirable side-effect\footnote{Due to space restrictions, we are unable to show this
in full effect}.
\iffalse
but Figure~\ref{fg:snippet} shows part of the generated
...
\iffalse
Figure~\ref{fg:witness} provides a snippet of
...
value of $C \geq 2$, using a completely automatic approach\footnote{The region $1.5\leq C < 2$ is a particularly hard problem - no results for this region are presented in Beyene, and our algorithm does not converge given a timeout of seven hours.}. %This fact alone,
